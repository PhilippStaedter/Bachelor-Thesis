...
plt.subplot(1,1,1)
not_bio = plt.scatter(num_x, num_p, alpha=0.8, c='blue', edgecolors='none', s=60)
bio = plt.scatter(num_x_bio, num_p_bio, alpha=0.8, c='red', edgecolors='none', s=60)
...
plt.ylabel('Number of parameters', fontsize=fontsize, fontweight='bold')
...
# title
plt.title('Model Density for state variables and parameters', fontsize=titlesize, fontweight='bold')
...
plt.legend((not_bio, bio, reg1[0], reg2[0]), ('all models from JWS only', 'all models from BioModels-Database only',
                                              'slope: ' + str(round(b[0],4)), 'slope: ' + str(round(e[0],4))), fontsize=fontsize, loc=2, frameon=False)
