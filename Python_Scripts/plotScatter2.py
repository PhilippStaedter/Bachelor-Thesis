...
# important paths
tolerance_path = '../bachelor_thesis/SolverAlgorithm/Adams'
...
for iCounter in range(0, int(len(linSol_files)/4)):
    ...
    if iCounter == 0:
        ...
        ax1.text(0.35, 1.05, 'linsol = DENSE', fontsize=fontsize, fontweight='bold', transform=ax1.transAxes)
        #ax1.get_xaxis().set_visible(False)
        ax1.tick_params(labelbottom=False)
        ax1.text(-0.12, 0.98, 'Simulation Time [ms]', fontsize=fontsize, fontweight='bold', transform=ax1.transAxes, rotation=rotation)

    elif iCounter == 1:
        ax1 = plt.axes([left + iCounter * row_factor, bottom, width, height])
        ax1.text(0.35, 1.05, 'linsol = SPGMR', fontsize=fontsize, fontweight='bold', transform=ax1.transAxes)
        #ax1.get_xaxis().set_visible(False)
        ...
        ax1.text(0.35, 1.05, 'linsol = SPBCG', fontsize=fontsize, fontweight='bold', transform=ax1.transAxes)
        #ax1.get_xaxis().set_visible(False)
        ax1.tick_params(labelbottom=False)
        ax1.text(-0.12, 0.98, 'Simulation Time [ms]', fontsize=fontsize, fontweight='bold', transform=ax1.transAxes, rotation=rotation)
        ...
        ax1.text(0.35, 1.05, 'linsol = SPTFQMR', fontsize=fontsize, fontweight='bold', transform=ax1.transAxes)
        #ax1.get_yaxis().set_visible(False)
        ax1.tick_params(labelleft=False)
        ax1.text(0.15, -0.3, 'Number of state variables', fontsize=titlesize, fontweight='bold', transform=ax1.transAxes)

    elif iCounter == 4:
        ax1 = plt.axes([left, bottom - (iCounter - 2) * column_factor, width, height])
        ax1.text(0.35, 1.05, 'linsol = KLU', fontsize=fontsize, fontweight='bold', transform=ax1.transAxes)
        ax1.text(0.15, -0.3, 'Number of state variables', fontsize=titlesize, fontweight='bold', transform=ax1.transAxes)
        ax1.text(-0.12, 0.98, 'Simulation Time [ms]', fontsize=fontsize, fontweight='bold', transform=ax1.transAxes, rotation=rotation)
    ...
    ax1.set_xscale('log')
    ax1.set_yscale('log')
    ax1.scatter(first_x, first_data, alpha=0.8, c='blue', edgecolors='none', s=30, label=str(linSol4legend_1) + ': ' + str(round(len(first_data)*100/166,2)) + ' %')
    ax1.scatter(second_x, second_data, alpha=0.8, c='red', edgecolors='none', s=30, label=str(linSol4legend_2) + ': ' + str(round(len(second_data)*100/166,2)) + ' %')
    ax1.scatter(third_x, third_data, alpha=0.8, c='yellow', edgecolors='none', s=30, label=str(linSol4legend_3) + ': ' + str(round(len(third_data)*100/166,2)) + ' %')
    ax1.scatter(fourth_x, fourth_data, alpha=0.8, c='grey', edgecolors='none', s=30, label=str(linSol4legend_4) + ': ' + str(round(len(fourth_data)*100/166,2)) + ' %')
...
# set global labels
plt.text(0.1, 4, 'Simulation time distribution of models for the Adams-Moulton Method', fontsize=titlesize, fontweight='bold', transform=ax1.transAxes)  # -60 , 350
...
# save figure
plt.savefig('../bachelor_thesis/New_Figures/Figures_study_4/NEW_NEW_Adams_Moulton.pdf')
