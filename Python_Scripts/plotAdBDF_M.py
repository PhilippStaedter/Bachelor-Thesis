...
for iValue in range(0, len(x_Adams)):
    if x_Adams[iValue] != 0 and y_BDF[iValue] != 0:
        ...
        bdf_zero_y.append(2000)
    elif x_Adams[iValue] == 0 and y_BDF[iValue] == 0:
        equal_zero_x.append(2000)
        equal_zero_y.append(2000)
...
ax = plt.axes()
z = range(0,2000)
plt1 = ax.scatter(adams_bdf_x, adams_bdf_y, s=70 , c='red', label='Adams-Moulton is faster than BDF: ' + str(round(len(adams_bdf_x)/166 * 100, 2)) + ' %', zorder=10, clip_on=False)
plt2 = ax.scatter(bdf_adams_x, bdf_adams_y, s=70, c='blue', label='BDF is faster than Adams-Moulton: ' + str(round(len(bdf_adams_x)/166 * 100, 2)) + ' %', zorder=10, clip_on=False)
plt3 = ax.scatter(equal_x, equal_y, c='grey', s=70, label='Both are equally fast: ' + str(round(len(equal_x)/166 * 100, 2)) + ' %', zorder=10, clip_on=False)
plt4 = ax.scatter(adams_zero_x, adams_zero_y, marker='D', s=120, facecolors='none', edgecolors='blue', label='Adams-Moulton failed completely to integrate the model: ' + str(round(len(adams_zero_x)/166 * 100, 2)) + ' %', zorder=10, clip_on=False)
plt5 = ax.scatter(bdf_zero_x, bdf_zero_y, marker='D', s=120, facecolors='none', edgecolors='red', label='BDF failed completely to integrate the model: ' + str(round(len(bdf_zero_x)/166 * 100, 2)) + ' %', zorder=10, clip_on=False)
...
ax.set_xscale('log')
ax.set_yscale('log')
ax.set_xlabel('Computation time for Adams-Moulton [ms]', fontsize=titlesize, fontweight='bold')
ax.set_ylabel('BDFs simulation time [ms]', fontsize=titlesize, fontweight='bold')
ax.set_title('Adams-Moulton vs. BDF Models', fontsize=titlesize, fontweight='bold', pad=30)
