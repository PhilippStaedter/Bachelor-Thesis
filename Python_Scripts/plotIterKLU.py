...
for iValue in range(0, len(x_Iter)):
    if x_Iter[iValue] != 0 and y_KLU[iValue] != 0:
        ...
        klu_zero_y.append(4000)
    elif x_Iter[iValue] == 0 and y_KLU[iValue] == 0:
        equal_zero_x.append(4000)
        equal_zero_y.append(4000)
...
ax = plt.axes()
z = range(0,4000)
plt1 = ax.scatter(iter_klu_x, iter_klu_y, s=70, c='red', label='Iterative solver is faster than KLU: ' + str(round(len(iter_klu_x)/166 * 100, 2)) + ' %', zorder=10, clip_on=False)
plt2 = ax.scatter(klu_iter_x, klu_iter_y, s=70, c='blue', label='KLU is faster than Iterative solver: ' + str(round(len(klu_iter_x)/166 * 100, 2)) + ' %', zorder=10, clip_on=False)
plt3 = ax.scatter(equal_x, equal_y, s=70, c='grey', label='Both are equally fast: ' + str(round(len(equal_x)/166 * 100, 2)) + ' %', zorder=10, clip_on=False)
plt4 = ax.scatter(iter_zero_x, iter_zero_y, marker='D', s=120, facecolors='none', edgecolors='blue', label='Iterative solver failed completely to integrate the model: ' + str(round(len(iter_zero_x)/166 * 100, 2)) + ' %', zorder=10, clip_on=False)
plt5 = ax.scatter(klu_zero_x, klu_zero_y, marker='D', s=120, facecolors='none', edgecolors='red', label='KLU failed completely to integrate the model: ' + str(round(len(klu_zero_x)/166 * 100, 2)) + ' %', zorder=10, clip_on=False)
plt6 = ax.scatter(equal_zero_x, equal_zero_y, marker='D', s=120, facecolors='none', edgecolors='grey', label='Both failed to integrate the model: ' + str(round(len(equal_zero_x)/166 * 100, 2)) + ' %', zorder=10, clip_on=False)
ax.plot(z)
ax.set_xlim([0.5, 4000])
ax.set_ylim([0.5, 4000])
ax.set_xscale('log')
ax.set_yscale('log')
ax.set_xlabel('Iterative solver simulation time [ms]', fontsize=titlesize, fontweight='bold')
ax.set_ylabel('KLU simulation time [ms]', fontsize=titlesize, fontweight='bold')
ax.set_title('Iterative Solver vs. KLU Models', fontsize=titlesize, fontweight='bold', pad=30)
