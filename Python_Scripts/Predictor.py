...
#benchmark_collection_path = '../bachelor_thesis/SolverAlgorithm/Benchmark_Input_Data_0_1_updated.tsv'
...
'''
if utils.multiclass.type_of_target(Y_testing_1) != 'multiclass':
    lab_enc = preprocessing.LabelEncoder()
    Y_testing_1_encoded = lab_enc.fit_transform(Y_testing_1)
    print(utils.multiclass.type_of_target(Y_testing_1.astype('int')))
    print(utils.multiclass.type_of_target(Y_testing_1_encoded))
'''
...
# clf = []
# for-loop
#X_training = [X1,..,X4], X_testing = X5
#clf.append(LogisticRegrssion())
#  OUTPUT =  clf[iLoop].fit(X_training, Y_training)
# test_score = clf[iLoop].predict(X_testing)            (train_score > test_score)
...
#cv = cross_validate(clf, X, Y_encoded, cv=5, return_estimator=True, return_train_score=True)
#a = 4
# score estimate (mean score / standard deviation)
...
###### logistic regression for simple validation of 20/80 ######
# perform logistic regression
clf = LogisticRegression()
clf.fit(X_training, Y_training)        #(X,Y)                #(X, Y_encoded)


# predict for testing models
#prediction = clf.predict(X_testing)   #(X_testing_1)    #(X)
# prediction2 = lab_enc.inverse_transform(prediciton)

'''
# get confusion metrics
try:
    cm = confusion_matrix(pd.Series.tolist(Y_testing['value']), prediction)           #(pd.Series.tolist(Y_testing_1['value']), prediction)            #(Y, prediction)
except:
    cm = confusion_matrix(Y_testing, prediction)                                      # (pd.Series.tolist(Y_testing_1_encoded['value']), prediction)
print(cm)

# get accuracy of prediction
try:
    accuracy = accuracy_score(pd.Series.tolist(Y_testing['value']), prediction) * 100   # (Y, prediction)
except:
...
results.to_csv('../bachelor_thesis/SolverAlgorithm/Results_continuous.tsv', sep='\t', index=False)
'''