...
for iCrossValidation in range(0, cv):
    ...
    ExpSigmoid = Fit.decision_function(X_testing)                                               # X-testing !!!
...
for iModel in range(0,166):
    ...
    best_df['best'][iModel] = sorted(list(A_6640_df['all'][iModel*40 : (iModel+1)*40]), reverse=True)[0]
    best_df['index'][iModel] = (np.argsort(list(A_6640_df['all'][iModel*40 : (iModel+1)*40]))[len(np.argsort(list(A_6640_df['all'][iModel*40 : (iModel+1)*40]))) - 1]) % 40
